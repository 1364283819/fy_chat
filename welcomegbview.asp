...
<td align="right"> <%if page-1>0 then%><a href="welcomegbview.asp?show=<%=show%>&page=<%=page-1%>">上一页</a><%else%><font color=666666>上一页</font><%end if%>　<%if page+1<=totalpage then%><a href="welcomegbview.asp?show=<%=show%>&page=<%=page+1%>">下一页</a><%else%><font color=666666>下一页</font><%end if%>　<a href=javascript:history.go(0) title=刷新本页>第<%=page%>页</a></td>
</tr>
</table>
</td>
</tr>
...
<td align="right"><a href="welcomegbwrite.asp?towho=<%=server.URLEncode(showfromwho(i))%>&title=<%=server.URLEncode(showtitle(i))%>">回复</a></td>
</tr>
</table>
</td>
...
<tr> 
<td><a href="welcomegbwrite.asp">书写留言</a></td>
<td> 
