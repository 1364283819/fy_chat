...
if ip="" then Response.Redirect "manerr.asp?id=320"
if querytype<>"1" then querytype="0"
...
conn.open connstr
if querytype="0" then
 sql="SELECT username,grade,times,value,regip,lastip,lasttime FROM reginfo WHERE kill='0' AND (lastip='" & ip & "' OR regip='" & ip & "')"
else
 sql="SELECT username,grade,times,value,regip,lastip,lasttime FROM reginfo WHERE kill='0' AND (lastip LIKE '%" & ip & "%' OR regip LIKE '%" & ip & "%')"
end if
...
<font color="#FF0000">【按ＩＰ查询帐号】</font> <br>
<a href="manacc.asp">返回</a></div>
<hr noshade size="1" color=009900>
<b>［注意事项］</b><br>
　　找到 <font color=red><%=totalrec%></font> 个帐号符合IP：<font color=red><%=ip%></font>。<%if totalrec>0 then%> 
<hr noshade size="1" color=009900>
<div align=center>
...
if page=i then
 ...
 Response.Write " <a href=manaccqueryipok.asp?ip=" & server.URLEncode(ip) & "&querytype=" & querytype & "&page=" & i & ">[" & i & "]</a>"
end if
next%></div>
<hr noshade size="1" color=009900>
