...
conn.open connstr
sql="SELECT username FROM reginfo WHERE kill='0' AND leave='0' AND times=1 AND DateDiff('d',lasttime,#" & sj & "#)>7"
...
rs.close
set rs=nothing
sql="DELETE FROM reginfo WHERE kill='0' AND leave='0' AND times=1 AND DateDiff('d',lasttime,#" & sj & "#)>7"
conn.Execute(sql)
conn.close
